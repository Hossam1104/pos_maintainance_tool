...
| Browse backup destination | raw configured folder in `BackupService` | `POST /api/v1/files/browse`, `POST /api/v1/files/handles` | `/backups` | read | Root allowlist, traversal/absolute/reparse rejection | New safety replacement — implemented Session 02 |
...
| Select restore source | raw ZIP/DB-path text boxes in `OperationsPage.xaml` | `POST /api/v1/uploads` or `POST /api/v1/files/browse` + `POST /api/v1/files/handles` | `/restore` | mutating | Upload bounds and opaque browse-handle tests | Replaces raw paths — file-browse half implemented Session 02 |
...
| Agent/API version and authorization check | none | `GET /api/v1/session` | shell (every route) | read | Unauthenticated/non-admin/admin cases | New — implemented Session 02 |
| Antiforgery token bootstrap | none | `GET /api/v1/antiforgery` | shell (every route) | read | Token issuance; downstream mutation rejected without it | New — implemented Session 02 |

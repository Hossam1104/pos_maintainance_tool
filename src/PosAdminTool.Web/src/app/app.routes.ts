...
export const routes: Routes = [
  ...
  { path: 'backups', title: 'Backups - DBS POS Admin', loadComponent: () => import('./features/backups-page.component').then((m) => m.BackupsPageComponent) },
  ...['restore','maintenance','downloads','activity'].map((path) => ({ path, title: `${path[0].toUpperCase()}${path.slice(1)} - DBS POS Admin`, data: { title: `${path[0].toUpperCase()}${path.slice(1)}`, detail: 'This workspace is prepared for its Agent-backed workflow in the next migration session.' }, loadComponent: page })),

...
export const routes: Routes = [
  ...
  ...['services','backups','restore','maintenance','downloads','activity'].map((path) => ({ path, title: `${path[0].toUpperCase()}${path.slice(1)} · DBS POS Admin`, data: { title: `${path[0].toUpperCase()}${path.slice(1)}`, detail: 'This workspace is prepared for its Agent-backed workflow in the next migration session.' }, loadComponent: page })),
